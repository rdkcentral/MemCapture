...
if ! mkdir -p /tmp/memcapture; then
    logger -t memcapture "ERROR: Failed to create directory"
    exit 1
fi
...
log_info() {
    logger -t memcapture -p user.info "$*"
}

log_error() {
    logger -t memcapture -p user.error "$*"
    echo "ERROR: $*" >&2
}

# Usage
log_info "Starting MemCapture memory collection"
